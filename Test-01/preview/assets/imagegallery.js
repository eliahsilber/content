...
zp.ImageGallery = function (){
	...
	function renderGallery(data, page, imagesPerPage, articleid, effectclass){
		...
		for (i = (page*imagesPerPage)-imagesPerPage; i < data.images.length; i++ ){
			...
			result += '	<a class="fancybox zpnolayoutlinkstyles" href="' + image.imagePath + '" title="' + image.description + '" data-htmltitle="' + encodeURI(image.encodedDescription) + '" data-fancybox-group="g' + igal.articleid + '">';
			...
			result += '	</a>';
			if ( decodeURI(image.encodedDescription) ){
				result += '	<div class="zpiacaption ' + image.thumbTitlePosition + '"><span>' + decodeURI(image.encodedDescription) + ' &nbsp;</span></div>';
			}
			result += '</div>\n';
